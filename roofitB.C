...
void roofitB(int doubly = 0, TString tree = "ntphi", int full = 1, int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString _varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 1, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100.)
{
	...
	for(int i=0;i<nBins_check;i++)
	{
		...
		if(doubly==0)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2)); 
		...
		ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		std::cout<< "Pass 2" << std::endl;
		//  RooRealVar* w = (RooRealVar*) data->addColumn(wFunc) ;
		//      RooDataSet wdata(data->GetName(),data->GetTitle(),data,*data->get(),0,w->GetName()) ;
		//dsMC = new RooDataSet(Form("dsMC%d",_count),"",skimtreeMC_new,RooArgSet(*mass, *pt));
		RooDataSet* dsMC_cut;
		if(doubly==0) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2), "pthatweightNew"); 
	...
	std::cout << "Now We do Splots" << std::endl;
		do_splot(w_val, ds_cut,centmin, centmax);
