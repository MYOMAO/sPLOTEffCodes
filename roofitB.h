...
RooFitResult *fit(TString variation, TString pdf,TString tree, TCanvas* c, TCanvas* cMC, RooDataSet* ds, RooDataSet* dsMC, RooDataHist* dh, RooDataHist* dhMC, RooRealVar* mass, RooPlot* &outframe, Double_t ptmin, Double_t ptmax, int isMC, bool isPbPb, Float_t centmin, Float_t centmax, TString npfit)
{
	...
	//	if(tree=="EffInfoTree")frameMC->SetXTitle("m_{J/#psi(#mu#mu)#it{#phi}(KK)} (GeV/c^{2})");
	if(tree=="ntKp")frameMC->SetXTitle("m_{J/#psiK^{#pm}} (GeV/c^{2})");
	//if(tree=="EffInfoTree")frameMC->SetXTitle("m_{J/#psi#it{#phi}} (GeV/c^{2})");
	if(tree=="EffInfoTree")frameMC->SetXTitle("m_{J/#psi#it{K^{+}K^{-}}} (GeV/c^{2})");
	...
	cMC->cd();
	double init_mean = 0;
	if(tree=="EffInfoTree") init_mean = BSUBS_MASS;
	...
	double mass_peak = 0;
	if(tree=="EffInfoTree") mass_peak = BSUBS_MASS;
	...
	*/
	...
	if(tree=="EffInfoTree")frame->SetMaximum(nsig.getVal()*0.8);
	...
	//	if(tree=="EffInfoTree")frame->SetXTitle("m_{J/#psi(#mu#mu)#it{#phi}(KK)} (GeV/c^{2})");
	...
	//	if(tree=="EffInfoTree")pull_plot->SetXTitle("m_{J/#psi(#mu#mu)#it{#phi}(KK)} (GeV/c^{2})");
	if(tree=="ntKp")pull_plot->SetXTitle("m_{J/#psiK^{#pm}} (GeV/c^{2})");
	//	if(tree=="EffInfoTree")pull_plot->SetXTitle("m_{J/#psi#it{#phi}} (GeV/c^{2})");
	if(tree=="EffInfoTree")pull_plot->SetXTitle("m_{J/#psi#it{K^{+}K^{-}}} (GeV/c^{2})");
	...
	if(drawOpt == 0) {
		leg = new TLegend(0.65,0.5,0.89,0.89,NULL,"brNDC");
	}
	...
	TLatex* texB;
	if(tree=="EffInfoTree") texB = new TLatex(0.21,0.8,"B^{0}_{s}");
...
void do_splot(RooWorkspace * w,  RooDataSet* data, double CentMin, double CentMax){

	cout << "DO SPLOT STARTS" << endl;

//	RooDataSet* data = (RooDataSet*) w->data((Form("ds%d",_count)));   
	RooAbsPdf* model = w->pdf((Form("model%d",_count)));
	...
	RooRealVar* BpYield = w->var(Form("nsig%d",_count));
	RooRealVar* BgYield = w->var(Form("nbkg%d",_count));
	...
	Double_t EffSplotWeight;

	TFile * foutputWeight = new TFile(Form("DataWeight/sPLOTEffWeight%.0f_%.0f.root",CentMin,CentMax),"RECREATE");
	foutputWeight->cd();
	
	TTree * EffWeightTreesPLOT = new TTree("EffWeightTreesPLOT","EffWeightTreesPLOT");
	EffWeightTreesPLOT->Branch("EffSplotWeight",&EffSplotWeight,"EffSplotWeight/D");
	...
	RooStats::SPlot* sData =  new RooStats::SPlot("sData","An sPlot",*data, model, RooArgList(*BpYield,*BgYield));


	cout << "Pass 5" << endl;

	cout << endl <<  "Yield of B2 is " 	<< BpYield->getVal() << ".  From sWeights it is " 	<< sData->GetYieldFromSWeight(Form("nsig%d",_count)) << endl;

	cout << "Yield of background is " 	<< BgYield->getVal() << ".  From sWeights it is "	<< sData->GetYieldFromSWeight(Form("nbkg%d",_count)) << endl;

	ofstream WeightFile("Weight.dat");
	...
	//if(0)
		cout << "y Signal Weight   "     << sData->GetSWeight(i,Form("nsig%d",_count)) << endl;;
		cout << "y Background Weight   "     << sData->GetSWeight(i,Form("nbkg%d",_count)) << endl;;

	
		WeightFile << "y Weight   "     << sData->GetSWeight(i,Form("nsig%d",_count)) << endl;
		
		EffSplotWeight = sData->GetSWeight(i,Form("nsig%d",_count));
